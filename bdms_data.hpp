...
public:
    ...
    const mxArray *getArray(const SessionID &sessionID, std::vector<std::string> &dataIDs);
};

const mxArray *BDMSDataManager::getArray(const SessionID &sessionID, std::vector<std::string> &dataIDs)
{
    ...
    const mxArray *outputBytes = mxCreateNumericArray(2, dims, mxUINT8_CLASS, mxREAL);
